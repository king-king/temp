 ...
 * 二维码,算法来自https://github.com/neocotic/qr.js
 */
library( function () {
	// Private constants
	// -----------------
	...
	// code.
	module.exports = function ( c2d, url, size, ECC ) {
		size *= 25; // Actual size of the QR code symbol and is scaled to 25 pixels (e.g. 1 = 25px, 3 = 75px).
		eccLevel = ECC_LEVELS[ECC || 'L']; // Determine the ECC level to be applied.

		var frame = generateFrame( url || '' ), // Generate the image frame for the given `value`.
			px = size / width << 0, // Determine the *pixel* size.
			i, j;
